...
function buildInsightsLocally(articles) {
  ...
  for (const [sector, arts] of Object.entries(sectors)) {
    ...
    insights.push({
      ...
      what:            `${count} signal${count > 1 ? 's' : ''} this cycle · sources: ${sources.slice(0, 2).join(', ')}`,
      topArticleTitle:  topArt.title,
      topArticleLink:   topArt.link   || '',
      topArticleSource: topArt.source || '',
    ...
    insights.push({
      ...
      narrativeCluster: ctx.narrativeCluster,
      narrative:       ctx.narrative,
      what:            `Trend signal across ${count} article${count > 1 ? 's' : ''} · sources: ${sources.slice(0, 2).join(', ')}`,
      topArticleTitle:  secArt.title,
      topArticleLink:   secArt.link   || '',
      topArticleSource: secArt.source || '',
      why:             ctx.trend.why,
